...
class Metadata:
    ...
    def get_text_chunks(text):
        ...
        for sentence in sentences:
            return_value += [("Type: "+str(label.value),"Confidence: "+str(label.score),"Phrase:"+str(label.data_point)) for label in sentence.get_labels()]

        return_value = [item[-1].split('\"')[1] for item in return_value if len(item[-1].split('\"')[1]) > 4]
        return_value = [' '.join([sub_item for sub_item in item.split(' ') if len(sub_item) > 4]) for item in return_value]
        return return_value
...
class Retr:
    ...
    def retrieve_filtered_context(text_splits,query,neural_net,top_k = 3):
        """
        retrieves top k context based on hybrid search
        """

        context = Retr.retrieve_context_neural(text_splits,query,neural_net)
        query_chunks_set = set(Metadata.get_text_chunks(query))
        filtered_context = [item for item in context if len(query_chunks_set.intersection(set(Metadata.get_text_chunks(item)))) > 1]
...
class Metadata_Retr_RAG:
    ...
    def run_demo():
        ...
        for article in random_articles:
            ...
            filtered_context = Retr.retrieve_filtered_context(text_splits,query,Neural_Net())
